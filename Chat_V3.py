...
class AIClient:
    ...
    def chat_with_ai(self, user_message, message_history):
        headers = {
...
class ChatApplication:
    ...
    # Modifiez la méthode send_message pour inclure les aperçus Excel
    def send_message(self, event=None):
        ...
        if self.attachments:
            full_message = user_message + "\n\n[Pièces jointes]\n"
            # On boucle et fait le traitement ici, avant d'envoyer le message.
            for file_id, attachment in self.attachments.items():
                file_name = attachment['name']
                file_type = attachment['type']
                #   On inclut le nom et le type de fichier. Vous pouvez ajouter plus de détails, si besoin,
                full_message += f"- {file_name} ({file_type})\n"

                # Vous *devez* inclure ici le code qui incorpore le contenu des fichiers,
                #  si ce n'est pas déjà fait.  Le bloc suivant est *très important* car il
                #  s'agit d'extraire le contenu des fichiers texte et autres en les ajoutant
                #  au message envoyé à l'IA. Adaptez ceci à vos besoins et à ce que vous
                #  voulez envoyer avec les attachments.
        ...
        #  À ce stade, full_message contient toutes les infos - nom, type, et contenu des fichiers (pour les textuels).
        #  Les widgets affichés seront retirés dans display_assistant_response
    ...
    def load_history_and_display(self):
        ...
        # S'assurer que le chat_box reste en mode NORMAL pour permettre la
        # sélection
        self.chat_box.config(state=tk.NORMAL)
    ...
    def append_message(self, role, message):
            ...
            if hasattr(self, 'chat_box2'):
                ...
                if role == "user":
                    self.chat_box2.insert(END, content, role)
                else:
