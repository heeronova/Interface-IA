...
class ChatApplication:
    ...
    def extract_excel_preview(self, file_path, max_rows_to_read=5, max_cols_to_read=30):
    ...
    def attach_file(self):
        ...
        if os.path.exists(file_path):
            ...
            preview = self.extract_excel_preview(file_path, max_rows_to_read=4, max_cols_to_read=8)
        ...
        if self.attachments:
            ...
            for file_id, attachment in self.attachments.items():
                ...
                for sheet_name, sheet_data in file_preview['first_rows'].items():
                   ...
                   if headers:
                       full_message += f"\n**Feuille '{sheet_name}':**\n"
                       full_message += f"**NOMS DES COLONNES:** "
                       # Un par ligne pour meilleure visibilité
                       for i, header in enumerate(headers):
                           full_message += f"\n   - Colonne {i+1}: {header}"
                           full_message += "\n"
                       
                       if sample:
                           full_message += "Données (5 premières lignes):\n"
                           for i, row in enumerate(sample[:5]):  # Limiter à 5 lignes
                               row_str = " | ".join([str(cell) for cell in row])
                               full_message += f"Ligne {i+1}: {row_str}\n"
    ...
    def fetch_and_display_response(self, user_message):
        try:
            ...
            # Pour le premier assistant, utiliser l'historique filtré
            assistant_response = self.ai_client.chat_with_ai(
                user_message, primary_history
                )
            ...
            if self.config["dual_chat"]:
                ...
                # Déboguer pour voir l'historique envoyé au second assistant
                logging.info(f"Historique pour le second assistant: {dual_chat_history}")
